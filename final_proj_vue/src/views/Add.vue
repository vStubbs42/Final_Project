<template>
  <v-container fluid class="contcolor">
    <v-row>
      ...
      <td class="text-xs-right">{{ props.item.locationType }}</td>
...
data () {
    ...
    methods:{
    checked(item) {
        if (item.campaign_name == this.campaignName) {
            item.checked = true
        }
        else {
            item.checked = false
    ...
    makeSelection() {
        ...
        Axios.post('http://localhost:8081/api/bookingAdd', JSON.stringify(params),
            {
            headers: {
  ...
  beforeMount(){
    ...
    Axios.get('http://localhost:8081/freeboards')
      .then(response=>{
